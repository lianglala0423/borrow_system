...
from db_final import ProductInfo, HistRecord, RegistForm # 匯入某個 class (某個資料表)
...
# @app.route('/flask-final-team4.esunaicloud.com/oauth/callback*', methods=['GET', 'POST'])
...
def search():
    ...
    # oant_uid = get_user_id()
    oant_uid = session['borrow_uid']
    ...
    return render_template('search.html', quotes=quotes, oant_uid=oant_uid) # 將查詢結果返回到前端
...
def borrow():
    ...
    if request.method == "POST":
        ...
        session['cur_rate'] = request.values.get('rate') \
                                if request.values.get('rate') != "" else None
        return redirect(url_for('borrow_submit', warn='submit', count=0))
    return render_template('borrow.html', quotes=quotes, oant_uid=oant_uid)

@app.route('/sendback', methods=['GET', 'POST'])
def sendback():
    # oant_uid = get_user_id() 
    oant_uid = session['borrow_uid']
    quotes = ProductInfo.query.filter_by(borrow_uid=oant_uid).all()       
    if request.method == "POST":
        session['product_id'] = request.values.get('product_id')
        session['product_category'] = request.values.get('product_category')
        session['product_name'] = request.values.get('product_name')
        session['cur_rate'] = request.values.get('rate') \
                                if request.values.get('rate') != "" else None
        return redirect(url_for('sendback_submit', warn='submit', count=0))
    return render_template('return.html', quotes=quotes, oant_uid=oant_uid)
...
def borrow_result(result):
    ...
    if result == 'success':
        ...
        res.append(f"錯誤次數已達3次，請確認您的借用流程是否正確。")
    # session.clear()
...
def sendback_result(result):
    ...
    if result == 'valid':
        ...
        res.append(f"{session['borrow_uid']} {session['borrow_uname']} 您好")
        res.append(f"錯誤次數已達3次，請確認您的歸還流程是否正確。")
    # session.clear()
